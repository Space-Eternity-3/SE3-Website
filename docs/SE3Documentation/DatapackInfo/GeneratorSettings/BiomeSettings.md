...
| 14  | struct=X                      | parametric | int <1;31>      | -       | Creates a [custom structure](../CustomStructures) in the middle of a biome. Tag `structural` is required to work. It is recommended to create empty space wheel in the middle of a biome using tag [ring.X.change](#ringxchange).                                    | Asteroids inside structure don't remove automatically. |
